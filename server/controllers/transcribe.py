...
#     word_durations = []
#     for word in words:
#         duration = len(word) * 0.07  # assuming an average reading speed of 2 characters per second
#         word_durations.append(duration)

#     # create a list to store the transcribed text with word durations
#     transcribed_text_with_durations = []
#     total_duration = 0.0
#     for i, word in enumerate(words):
#         if i % 6 == 0 and i != 0:
#             transcribed_text_with_durations.append((total_duration, " ".join(words[i-6:i])))
#             total_duration = 0.0
#         total_duration += word_durations[i]
    
#     # append the remaining words if any
#     if total_duration > 0.0:
#         transcribed_text_with_durations.append((total_duration, " ".join(words[len(words)-6:])))

#     # return the transcribed text with word durations
#     return transcribed_text_with_durations

# if __name__ == '__main__':
#     audio_path = sys.argv[1]
#     transcribed_text_with_durations = transcribe_audio(audio_path)

#     # print the transcribed text with word durations
#     print("Transcription with Word Durations:")
#     compteur=0
#     for duration, segment in transcribed_text_with_durations:
#         compteur+=duration
#         print(f"Duration: {compteur} seconds, Segment: {segment}")
